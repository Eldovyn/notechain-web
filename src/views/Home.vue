...
const openDialog = (card?: NoteCard) => {
  if (card) {
    selectedCard.value = card as any;
    form.title = card.title ?? "";
    form.content = card.content ?? "";
    form.summary = card.summary ?? "";
  } else {
    selectedCard.value = null as any;
    form.title = "";
    form.content = "";
    form.summary = "";
  }
...
const onSubmit = () => {
  ...
  try {
    ...
    cards.value = [newCard as any, ...cards.value];
...
const onDelete = () => {
  ...
  try {
    const id = (selectedCard.value as any)?.id;
    if (!id) {
      toast.error("Note tidak ditemukan");
      return;
    }
...
const onEdit = () => {
  ...
  try {
    ...
    toast.success("Note berhasil diperbarui");
    closeDialog();
    selectedCard.value = null as any;
  } finally {
    isSubmitting.value = false;
      ...
      <section class="sticky top-16 z-10 border-b bg-slate-50/80 backdrop-blur pt-3">
        <div class="mx-auto w-full max-w-6xl px-4 sm:px-6 py-4">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div
              class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end sm:w-[60%]"
            >
      ...
      <section class="mx-auto w-full max-w-6xl px-4 sm:px-6 py-6">
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            ...
            <CardContent
              class="text-sm text-slate-600 leading-relaxed line-clamp-4 min-h-[92px]"
            >
              {{ card.content }}
            </CardContent>
      ...
      <Dialog v-model:open="isOpen">
        <DialogContent class="sm:max-w-xl">
          ...
          <form
            ...
            @submit.prevent="!selectedCard && onSubmit()"
          >
            ...
            >
              ...
              <div
                class="flex flex-col gap-3 sm:flex-row sm:justify-end sm:ml-auto w-full"
              >
